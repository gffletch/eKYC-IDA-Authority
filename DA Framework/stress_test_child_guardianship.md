...
*Evaluating the delegation components (Relationship, Authority, Obligations, Constraints) of the Reference Architecture against the OIDF Delegated Authority Policy Framework for Child Rights-Respecting Digital Environments (O'Connell & Curtis, April 2026).*
...
|---|---|
| Authority claim | Authority (derivative authority section) |
| Verification against authoritative sources | Authority (source authority + verified_claims envelope) |
| Representation of authority chains | Relationship + Authority (`prior_authority_ref`) |
| Time-boundedness and review | Relationship (`exp`, lifecycle); Constraints (temporal predicates) |
| Revocation and transfer | Relationship (`rev_endpoint`, `rel_status`) — but with notable gaps; see §3.3 |
| Consent dashboard and audit trail | Out of scope for the delegation components; flagged for the execution layer (Permissioned Capabilities, Protected Access) |
...
For example, the residential care manager's authority over a 14-year-old's digital life is a **scope domain** (`https://cityhospital.example/scope/digital-guardianship/<placement_id>`), expressed in Authority. The specific operational decisions — "approve TikTok for educational research only, daily 30-minute window, ages-13-15 peer matching, no monetization features" — are Obligations and Constraints in our Task JWT. The Care Plan itself is the underlying policy document referenced via Tier 2 `policy_ref`.
...
5. **Scope-domain abstraction rules** — under the privacy-preserving profile, scope_domain URIs MUST be drawn from a registered, abstract vocabulary (e.g., `urn:scope:guardianship:platform-management`) rather than deployment-specific URIs that leak context.
...
This may belong in the execution layer (Permissioned Capabilities, Protected Access) rather than the establishment-layer components, but it must be flagged in the delegation components because the privacy properties of the establishment-layer JWTs depend on this role existing.
...
**Recommendation:** Add to Relationship a discussion of **graduated authority over time** as a deployment pattern, with both implementation options described. Do not normatively prefer one — the framework's text seems compatible with either, and the choice depends on the issuing intermediary's operational model. This should be flagged in Section 10 (Open Questions) of the Reference Architecture.

### 3.5 The two-key-worker / consensus pattern

The framework distinguishes three decision tiers (routine / significant / strategic) where strategic decisions require **full consensus** among multiple authorized parties. Our Relationship's quorum (`min_members`) handles the structural requirement, but the framework adds two refinements:
...
Our current architecture handles (1) imperfectly — we'd need to express it as multiple Relationships (one per decision tier) or as Tier 2 policy_ref. (2) is handled cleanly by our policy-defined group model, but only if we extend it to express **structured role-tuples** rather than single-role policies.

**Recommendation:** In Relationship, extend the policy-defined group descriptor to support a **role-tuple** sub-type, where membership requires presenting credentials for *each* of N specified roles. The Tier 1 vocabulary stays simple; the Tier 2 escape hatch absorbs the more complex consensus-protocol cases.
...
## 4. Where the Framework Reveals Issues for the Execution Layer

The document gives us advance warning of issues we'll face when designing the execution layer:

### 4.1 The Permissions Dashboard is execution-layer territory

The framework's "Permissions Dashboard" — held by a regulated intermediary, accessible to the verified guardian, the IRO, and the child in age-appropriate format, with timestamped authorization proofs — is exactly the kind of artifact that lives at the execution layer. It is not a JWT; it is a stateful, queryable record. Permissioned Capabilities and Protected Access should be designed with this dashboard concept as a reference use case, and the `consent_dashboard_ref` field in the Authority should be specified at that point.

### 4.2 AI Intention Mandates as a Permissioned Capabilities concept

The framework's "AI Intention Mandate" (defined in Appendix A) is a structured declaration setting out the purposes, limits, and safeguards under which an AI system may act on behalf of a child or guardian. It is "cryptographically bound to the credentials or tokens used by each AI system" — i.e., it lives at the execution layer, where capabilities are actually issued to AI workloads. This is a strong fit for the Permissioned Capabilities design and validates our approach of separating the establishment-layer delegation components from the execution layer (Permissioned Capabilities, Protected Access).

### 4.3 Dispute and chargeback mechanisms

The framework describes "dispute resolution and chargebacks where harm or misuse occurs" as a required execution-layer capability. Permissioned Capabilities must support ergonomic revocation, and Protected Access must support credentialed dispute processes. This is more concrete operational guidance than we had before and should be incorporated into the execution-layer design phase.
...
5. **Graduated-authority-over-time** as a documented deployment pattern in Relationship
...
These additions should be incorporated into the Reference Architecture before we proceed to Permissioned Capabilities and Protected Access, because the privacy-preserving profile requirements ripple into the execution layer's design.
